...
import "../circuits/contract/shielded_attack/plonk_vk.sol";
...
contract ShieldedAttack is MerkleTreeWithHistory {
	...
	*/
	...
	function verifyShieldedAttack(
		bytes calldata proof,
		bytes32[] calldata publicInputs
	) public view returns (bool) {

		// 1) generate zk proof of missle attack with secret coordinates + tokenId with nargo
		// then verify proof before settings effects: damage, burning missile (add to nullifier list), etc.
		bool proofResult = verifier.verify(proof, publicInputs);
